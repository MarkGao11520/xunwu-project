...
function drawRegion(map,regionList) {
    ...
    for(var i=0;i<regionList.length;i++){
        ...
        var textContent = '<p style="margin-top: 20px;pointer-events: none">'+regionList[i].cn_name+'</p>'
        ...
        textLabel.addEventListener('mouseover',function (event) {
            boundaryShowAndHide(event,true,polygonContext);
        });

        textLabel.addEventListener('mouseout',function (event) {
            boundaryShowAndHide(event,false,polygonContext);
        });
...
function boundaryShowAndHide(event,isShow,polygonContext) {
    var label = event.target;
